...
// 1. Projects Routes
app.get("/api/employee-management/projects", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  try {
...
app.post("/api/employee-management/projects", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  ...
  try {
    const { data, error } = await supabase
      .from("projects")
      .upsert({ id, name, description, created_at: new Date().toISOString() })
      .select();
    if (error) throw error;
    res.json(data?.[0] || { success: true });
  } catch (err: any) {
    console.error("Save project error:", err);
    res.status(500).json({ error: err.message || "Failed to save project database." });
  }
});

app.delete("/api/employee-management/projects/:id", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const { id } = req.params;
  try {
    ...
    res.status(500).json({ error: err.message || "Failed to delete project database." });
  }
});

// 2. Employees Routes
app.get("/api/employee-management/employees", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  ...
  try {
    ...
    const mapped = (data || []).map((emp: any) => ({
      ...
      name: emp.name,
      nameArabic: emp.name_arabic,
      iqamaNo: emp.iqama_no,
      expiryDate: emp.expiry_date,
      dob: emp.dob,
    ...
    res.status(500).json({ error: err.message || "Failed to retrieve employees." });
  }
});

app.post("/api/employee-management/employees", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const emp = req.body;
  try {
    const { data, error } = await supabase
      ...
      .upsert({
        id: emp.id,
        project_id: emp.projectId,
        name: emp.name,
        name_arabic: emp.nameArabic,
        iqama_no: emp.iqamaNo,
        ...
        created_at: emp.createdAt ? new Date(emp.createdAt).toISOString() : new Date().toISOString()
      })
      .select();
    if (error) throw error;
    res.json(data?.[0] || { success: true });
    ...
    res.status(500).json({ error: err.message || "Failed to save employee profile." });
  }
});

app.delete("/api/employee-management/employees/:id", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const { id } = req.params;
  try {
...
// 3. Timesheet Routes
app.get("/api/employee-management/timesheets", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const { projectId, year, month } = req.query;
  if (!projectId || !year || !month) {
    res.status(400).json({ error: "Missing required query parameters: projectId, year, month." });
    return;
  }
  try {
    const { data, error } = await supabase
    ...
    // Map snake_case database schema to frontend camelCase
    const mapped = (data || []).map((ts: any) => ({
      ...
      absentDays: ts.absent_days,
      otherAllowances: Number(ts.other_allowances),
      deductions: Number(ts.deductions),
      notes: ts.notes
    }));
    ...
    res.status(500).json({ error: err.message || "Failed to retrieve timesheets." });
  }
});

app.post("/api/employee-management/timesheets", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const ts = req.body;
  try {
    const { error } = await supabase.from("timesheets").upsert({
      ...
      year: ts.year,
      month: ts.month,
      days_worked: ts.daysWorked,
      overtime_hours: ts.overtimeHours,
      absent_days: ts.absentDays,
      other_allowances: ts.otherAllowances,
    ...
    res.status(500).json({ error: err.message || "Failed to save timesheet entry." });
  }
});

// 4. Secure Encrypted Card Image Routes
app.get("/api/employee-management/images/:employeeId", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  ...
  try {
    ...
    console.error("Fetch employee image error:", err);
    res.status(500).json({ error: err.message || "Failed to retrieve card scan." });
  }
});

app.post("/api/employee-management/images", authMiddleware, async (req, res) => {
  const supabase = getSupabaseOrError(res);
  if (!supabase) return;
  const { employeeId, imageBase64 } = req.body;
  if (!employeeId || !imageBase64) {
    res.status(400).json({ error: "Missing required fields employeeId or imageBase64." });
    return;
  }
  try {
    const { error } = await supabase.from("employee_images").upsert({
      id: employeeId,
    ...
    console.error("Save employee image error:", err);
    res.status(500).json({ error: err.message || "Failed to save card scan." });
  }
});
